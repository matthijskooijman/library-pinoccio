...
static uint8_t send_ping(uint16_t dst) {
    NwkFrame_t *frame;
    frame = nwkFrameAlloc();
    if (!frame) {
...
static uint8_t send_select_memory(uint16_t dst, MemType memtype) {
    NwkFrame_t *frame;
    frame = nwkFrameAlloc();
    if (!frame) {
...
static uint8_t send_set_address(uint16_t dst, uint32_t address) {
    NwkFrame_t *frame;
    frame = nwkFrameAlloc();
    if (!frame) {
...
static uint8_t send_data(uint16_t dst, uint8_t *buf, uint32_t flash_addr, uint8_t size) {
    NwkFrame_t *frame;
    frame = nwkFrameAlloc();
    if (!frame) {
//    speol(F("Failed to send ping"));
...
static uint8_t send_exit(uint16_t dst) {
    NwkFrame_t *frame;
    frame = nwkFrameAlloc();
    if (!frame) {
...
static cn_cbor *ping(cn_cbor *args) {
...
static cn_cbor *start(cn_cbor *args) {
    uint16_t address = (uint16_t) cn_cbor_mapget_string(args, "address")->v.uint;
...
//    speol(F("FAIL"));
    ...
    target = address;
    memtype = dryrun ? MemType::NOP : MemType::FLASH;
...
static cn_cbor *block(cn_cbor *args) {
...
static cn_cbor *clone(cn_cbor *args) {
...
static cn_cbor *end(cn_cbor *args) {
    if (state != State::IDLE) {
//    speol(F("Ota operation in progress?"));
//    speol(F("FAIL"));
        scout.led.red();
